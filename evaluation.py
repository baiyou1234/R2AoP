...
pred_dir = "/home/cyf/LLM/step1/out_png/pred/xiaorong_4/fold0"
gt_dir   = "/home/cyf/LLM/step1/out_png/gt/xiaorong_4/fold0"
...
for f in pred_files:
    ...
    gt_path   = os.path.join(gt_dir, f)   # 用同名文件匹配

    if not os.path.exists(gt_path):
        print(f"⚠️ 没找到 GT 文件: {gt_path}")
        continue

    # 读图并 resize
    pred_png = cv2.imread(pred_path, cv2.IMREAD_GRAYSCALE)
    pred_png = cv2.resize(pred_png, (256, 256), interpolation=cv2.INTER_NEAREST)
    gt_png   = cv2.imread(gt_path, cv2.IMREAD_GRAYSCALE)
    gt_png   = cv2.resize(gt_png, (256, 256), interpolation=cv2.INTER_NEAREST)

    pred_arr = np.array(pred_png, dtype=np.uint8)
    gt_arr   = np.array(gt_png, dtype=np.uint8)
    
    pred_arr = pred_arr.copy()
    gt_arr   = gt_arr.copy()

    pred_arr[pred_arr == 127] = 1
    pred_arr[pred_arr == 254] = 2
    pred_arr[pred_arr == 255] = 2
    gt_arr[gt_arr == 127] = 1
    gt_arr[gt_arr == 254] = 2
    gt_arr[gt_arr == 255] = 2
    mask1 = (pred_arr == 1).astype(np.uint8) * 255
    mask2 = (pred_arr == 2).astype(np.uint8) * 255

    evaluator = Evaluation(pred_arr, gt_arr)
    result = evaluator.process()   # 这里返回的是 {"score":..., "loss":..., "dice":..., ...}


    print(f'process the {f}')
    print(result)
    #raise ValueError
    results.append(result)
if len(results) == 0:
    raise RuntimeError("No results were collected during evaluation. Please check dataloader or inference.")
# 获取所有指标名（除了像 aop_records 这种list的，可以剔除）
all_metrics = [k for k in results[0].keys() if not isinstance(results[0][k], list)]
