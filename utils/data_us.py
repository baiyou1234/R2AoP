...
class JointTransform2D:
    ...
    def __call__(self, image, mask):
        ...
        if image.ndim == 3 and image.shape[2] == 1:
            ...
            mask = np.squeeze(mask,axis=2)  # shape (540,800)
        #raise ValueError(image.shape, mask.shape)
...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, i):
        ...
        id_ = id_.strip()
        parts = [p for p in id_.split("/") if p != ""]

        if len(parts) >= 3:
            class_id0, sub_path, filename = parts[0], parts[1], parts[2]
            rel_no_suffix = "/".join(parts)  # keep full relative id
        elif len(parts) == 2:
            class_id0, sub_path, filename = parts[0], "", parts[1]
            rel_no_suffix = "/".join(parts)
        elif len(parts) == 1:
            class_id0, sub_path, filename = "", "", parts[0]
            rel_no_suffix = parts[0]
        else:
            # empty line or invalid
            raise ValueError(f"Empty/invalid id line in id_list_file: {repr(id_)}")
        ...
        #label_path = os.path.join(self.dataset_path)
        ...
        #mask = sitk.ReadImage(os.path.join(label_path, filename + '_mask.mha'))
        mask = sitk.GetArrayFromImage(mask).astype(np.uint8)

        # --- 关键：把 0/127/254 映射为 0/1/2，兼容 nnUNet2D 的 CE ---
        u = np.unique(mask)
        # 如果数据本来就是 0/1/2，这段不会改变任何东西
        if np.max(u) > 2:
            # 常见映射：0->0, 127->1, 254->2
            mask2 = np.zeros_like(mask, dtype=np.uint8)
            mask2[mask == 127] = 1
            mask2[mask == 254] = 2
            # 保底：如果还有别的值，直接当背景（也可以改成 raise 报错）
            mask = mask2
